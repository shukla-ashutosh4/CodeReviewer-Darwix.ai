...
class MockGroq:
    def __init__(self, api_key: str = ""):
        ...
        # emulate nested .chat.completions.create(...)
    ...
    def _mock_create(self, messages=None, model=None, temperature=None, max_tokens=None):
        # For the code-feedback call we expect to return a JSON with fields:
        # positive_rephrasing, the_why, suggested_improvement, resource_link
        # We'll craft a simple but sensible JSON response based on the user's prompt contents.
        user_msg = ""
        if messages:
        ...
        # A naive heuristic: if prompt contains "Positive Rephrasing", produce JSON
        if "Positive Rephrasing" in user_msg or "Respond only with valid JSON" in user_msg:
            # produce a JSON string as the model would
            mock_json = {
                "positive_rephrasing": "Nice work — the logic is solid. We can make this a bit more efficient and clearer by simplifying the loop and naming.",
                "the_why": "Combining simple boolean checks and using clearer variable names improves readability and can use built-in language constructs to be faster and more idiomatic.",
            ...
            # return an object that resembles the Groq response shape
            return SimpleNamespace(choices=[SimpleNamespace(message=SimpleNamespace(content=json.dumps(mock_json)))])
        else:
            # For holistic summary calls, return plain text string
            summary_text = (
                "Great work! You've implemented functional logic and with a few changes (readability, naming, and "
                "idiomatic constructs) the code will be more maintainable and efficient. Keep iterating — small wins compound."
            )
...
# Accessible color tokens and improved theme:
CUSTOM_CSS = """
<style>
:root{
    --bg: #0f1724;              /* near-black background for contrast */
    --surface: #0b1220;         /* slightly lighter card background */
...
# ------------------- Main App Logic (keeps your original functionality) -------------------
class EmpatheticCodeReviewer:
    def __init__(self, groq_api_key: str):
        """Initialize the Empathetic Code Reviewer with Groq API key."""
        # Use the real Groq client if available, else the mock
        if GroqAvailable and Groq is not None:
            try:
                self.client = Groq(api_key=groq_api_key)
            except Exception:
                # fallback to mock if real client fails to initialize
